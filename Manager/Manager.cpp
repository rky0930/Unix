...
ManagerMain g_app;
map<string, string> process_info_map;
comm_log c_log;
...
int ManagerMain::process()
{
    ...
    for(itMap = process_info_map.begin(); itMap != process_info_map.end(); itMap++) {

        config_name   = itMap->first;
        config_value  = itMap->second;
        cout<<config_name<<": "<<config_value<<endl;
        
        for (int j=1;j<atoi(process_info_map[config_name+"_COUNT"].c_str())+1; j++) {
